...
@section('links')
    ...
    <div class="col-lg-6 col-5 text-right">
        <a href="/dashboard/admin/post/add" class="btn btn-sm btn-neutral">Add Post</a>
                            ...
                            <td class="text-right">
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right dropdown-menu-arrow">
                                        <form action="{{ url('/dashboard/admin/post/edit') }}" method="GET">
                                            @csrf
                                            <input value="{{ $item->id }}" type="hidden" name="id">
                                            <input value="{{ $item->image }}" type="hidden" name="old_image">
                                            <button type="submit" class="dropdown-item">Edit</button>
                                        </form>
                                        <form action="{{ url('/delete-admin-post') }}" method="POST" onsubmit="return confirm('Are you sure want to delete this post?')">
                                            @csrf
                                            <input value="{{ $item->id }}" type="hidden" name="id">
                                            <input value="{{ $item->image }}" type="hidden" name="old_image">
